...
import { SimulationWorkerClient, type WorkerLike } from './simulationWorkerClient';
...
function createClient(): { client: SimulationWorkerClient; worker: MockWorker } {
...
describe('SimulationWorkerClient', () => {
  ...
  it('resolves the promise when worker responds with result', async () => {
    ...
    const postedMessage = worker.postMessage.mock.calls[0][0];
